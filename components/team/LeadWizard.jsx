'use client';
// Team-app "Add Lead" wizard — an employee's own version of the Partner app's Punch New Lead
// flow (components/partner/LeadWizard.jsx), reached from the same kind of center FAB in the
// bottom nav (see components/team/ui.jsx). Deliberately a near-duplicate of the Partner wizard
// rather than a shared/parameterized component — same reasoning as every other Team screen
// (TeamHome, LeadsScreen, ProfileScreen): borrow the Partner app's UI primitives and icons
// directly (components/partner/ui.jsx / components/partner/icons.jsx, both already imported by
// components/team/ui.jsx), but keep the screen itself local so a change to one app's flow never
// risks the other's.
//
// The only real differences from the Partner wizard: it posts source: 'employee_app' (so
// app/api/leads/route.js credits the lead to the logged-in employee via addedByEmployeeId,
// never a client-supplied id — same trust model as partnerId), its copy says "Add Lead" instead
// of "Punch New Lead", and every route it navigates back to is under /team instead of /partner.
// Field-for-field it's identical, including "How did you connect?" — just as relevant for an
// employee logging a walk-in, cold call, or referral they personally handled.
...
export default function TeamLeadWizard() {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch('/api/leads', {
        ...
        body: JSON.stringify({ ...form, source: 'employee_app' }),
      });
    ...
    return (
      <div className="hp-success-wrap">
        ...
        <div className="hp-next-block">
          ...
          <div className="hp-check-item"><IconCheck size={16} /> You'll get credit for this lead in your activity</div>
        </div>

        <Button block onClick={() => router.push('/team/home')}><IconHome size={16} /> Back to Dashboard</Button>
      </div>
    );
  }
  ...
  return (
    <>
      <ScreenHeader title="Add Lead" onBack={() => (step === 0 ? router.push('/team/home') : setStep((s) => s - 1))} />
