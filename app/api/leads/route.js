...
export async function POST(request) {
  try {
    ...
    if (source === 'partner_app') {
      ...
      // partner's storefront/business name (shopName — the "Partner Business Name" field in
      // My Profile), falling back to their account name, not a stranger's personal name (see
      // also lib/attribution.js's referrerNoteFor, which follows the same priority).
      addedByLabel = `${partner.shopName || partner.businessName || partner.name || 'Our partner'}, Heseos Partner`;
