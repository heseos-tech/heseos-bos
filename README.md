...
2. **QR image generation** — `/wa/[ref]` gives you the target URL; turning that into a printable
...
## Mobile App (Partner) — Android & iOS

The Partner app is a real native shell (Capacitor) around the live `/partner` pages at
`heseos-bos-psi.vercel.app` — it's not a separate codebase, so every change you ship to the web
app shows up in the native app too, with no app-store update needed (only icon/name/native-plugin
changes require a new build). The native projects live in `android/` and `ios/`, generated by
`scripts/setup-mobile-app.sh` (safe to re-run any time you want to regenerate icons/splash or
re-sync config — it won't wipe your keystore or signing setup).

### Android — build & release

One-time setup, on your own Mac, outside this repo:

```bash
keytool -genkeypair -v -keystore ~/heseos-partner-release.keystore -alias heseos-partner -keyalg RSA -keysize 2048 -validity 10000
```

`keytool` will ask for a keystore password and a key password (can be the same), plus your
name/org details for the certificate — none of that is sensitive, but the two passwords and the
`.keystore` file itself are: **store the file somewhere outside this repo and back it up
somewhere safe.** If you ever lose it, you can never publish an update to the same Play Store
listing again — Google cannot reset this for you.

Then copy `android/keystore.properties.example` to `android/keystore.properties` (already
git-ignored) and fill in the real path + passwords:

```
storeFile=/Users/yourname/heseos-partner-release.keystore
storePassword=...
keyAlias=heseos-partner
keyPassword=...
```
...
Output: `android/app/build/outputs/bundle/release/app-release.aab` — upload that `.aab` to Play
Console → your app → Production (or a testing track first).

### iOS — build & release (needs Xcode)

1. Open `ios/App/App.xcodeproj` in Xcode (not a `.xcworkspace` — this project uses Swift Package
   Manager, not CocoaPods).
2. Select the **App** target → **Signing & Capabilities** tab → choose your Apple Developer
   **Team** → leave "Automatically manage signing" checked. You'll need to be signed in to Xcode
   with your Apple ID (Xcode → Settings → Accounts) for your Team to show up here.
3. **Product → Archive** (pick "Any iOS Device" as the run destination first — Archive is greyed
   out on a simulator destination).
4. When the archive finishes, the **Organizer** window opens automatically → **Distribute App** →
   **App Store Connect** → **Upload** → follow the prompts (defaults are fine).
5. The build then shows up in App Store Connect (appstoreconnect.apple.com) under your app →
   TestFlight (usually within 15–30 min after Apple finishes processing) — from there you can send
   it to TestFlight testers or submit it for App Store review.

Bumping the version for a future release: edit `MARKETING_VERSION` (user-facing version, e.g.
"1.1") and `CURRENT_PROJECT_VERSION` (build number, must increase every upload) in Xcode's
target **General** tab, and the matching `versionName`/`versionCode` in
`android/app/build.gradle`, before re-archiving/re-bundling.
