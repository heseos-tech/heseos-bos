...
// lead recap with no explanation of why they're seeing it. Bumped to a new id again rather than
// mutating the v3 row in place — the self-healing seed (ensureHeseosReturningFlow below) never
// overwrites an existing flow row because a tenant may have hand-edited it in the Flow Builder
// since it was first seeded, so a change like this one needs a fresh id to actually reach any
// tenant who already got an earlier seed; older rows, if they exist, are simply orphaned and
// harmless.
export const HESEOS_RETURNING_FLOW_ID = 'heseos_returning_customer_v4';
