  ...
  const nodes = [
    { id: 'start', type: 'start', x: 60, y: 220, data: {} },
    {
      ...
      data: {
        ...
        // HESEOS brand poster — see lib/heseosDefaultFlow.js's matching n_greet node and
        // lib/botFlowEngine.js's play() for what data.imageAfter does. Sent 4s after this
        // greeting, then n_menu (below) follows immediately with no further delay.
...
// One-off migration, applied in place — same mechanic and reasoning as
// lib/heseosDefaultFlow.js's withHeseosGreetImage (not shared code since these are two
// independent flows with their own node ids/shape, but the exact same idea): n_greet's
// data.imageAfter (the HESEOS brand poster) was added after this flow first shipped, so an
// already-seeded tenant's row needs a targeted, in-place patch rather than a full reseed.
