...
import { sendInteractiveMenu, numberedMenuText } from '@/lib/botMenu';
...
// menu (raw {label, icon} rows, used for matching a reply back) -> items (WhatsApp-ready
// {id, label} pairs — see lib/botMenu.js) with a stable, positional id since these configured
// rows don't carry one of their own.
function menuItems(menu) {
  return menu.map((m, i) => ({ id: `menu_${i}`, label: `${m.icon || ''} ${m.label}`.trim() }));
}

function menuText(tenant) {
  const menu = activeMenu(tenant);
  return { menu, text: `Here's what I can help with:\n${numberedMenuText(menuItems(menu))}` };
}

// Same id-first-then-typed-text matching as lib/botFlowEngine.js's matchMenuOption — see that
// file's comment for why. `replyId` here is `menu_<index>` (see menuItems above).
function matchMenuOption(menu, raw, replyId) {
  if (replyId) {
    const m = /^menu_(\d+)$/.exec(replyId);
    if (m && menu[Number(m[1])]) return menu[Number(m[1])];
  }
...
const LANG_PROMPT_HEAD = 'Please choose your language 👇\n*अपनी भाषा चुनें*';

// choices -> WhatsApp-ready {id, label} items, id-encoding the language code so
// matchLanguage below can recover it straight from a tap without re-parsing a title.
function langItems(choices) {
  return choices.map((c) => ({ id: `lang_${c.code}`, label: c.label }));
}

function langPromptText(tenant) {
  const choices = languageChoices(tenant);
  return `${LANG_PROMPT_HEAD}\n${numberedMenuText(langItems(choices))}`;
}

// Same id-first-then-typed-text matching as lib/botFlowEngine.js's matchMenuOption.
function matchLanguage(tenant, raw, replyId) {
  const choices = languageChoices(tenant);
  if (replyId) {
    const m = /^lang_(.+)$/.exec(replyId);
    if (m) {
      const found = choices.find((c) => c.code === m[1]);
...
export async function runBotTurn(tenant, chat, inboundText, replyId) {
  const multiLang = (tenant.languages || ['en']).length > 1;
  let stage = chat.stage || (multiLang ? 'lang' : 'welcome');

  if (stage === 'lang') {
    const picked = matchLanguage(tenant, inboundText, replyId);
    if (!picked) {
      const choices = languageChoices(tenant);
      await sendInteractiveMenu(tenant, chat, { headText: LANG_PROMPT_HEAD, archivalText: langPromptText(tenant), items: langItems(choices) });
      return;
    }
  ...
  if (stage === 'welcome') {
    ...
    const { menu, text } = menuText(tenant);
    await sendInteractiveMenu(tenant, chat, { headText: "Here's what I can help with:", archivalText: text, items: menuItems(menu) });
  ...
  if (stage === 'menu') {
    const { menu, text } = menuText(tenant);
    const picked = matchMenuOption(menu, inboundText, replyId);
    if (picked) {
    ...
    await sendInteractiveMenu(tenant, chat, { headText: "Here's what I can help with:", archivalText: text, items: menuItems(menu), prefix: "Sorry, I didn't quite get that." });
