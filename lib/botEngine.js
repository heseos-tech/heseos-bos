...
function welcomeText(tenant, lang) {
...
// Sends `body` from the bot, persists it exactly like a human-sent message so the Inbox thread
// (components/bot/InboxScreen.jsx) shows one continuous conversation either way.
async function botReply(tenant, chat, body) {
...
export async function runBotTurn(tenant, chat, inboundText) {
  ...
  if (stage === 'welcome') {
    ...
    await botReply(tenant, chat, welcomeText(tenant, lang));
