...
export const EMPLOYEE_ROLES = ['presales', 'sales_engineer', 'operations', 'marketing', 'management', 'admin'];
...
// Product catalogue categories (Admin -> Products, and the quotation builder's product
// picker). Mirrors the same vocabulary PRODUCT_INTEREST above already uses for what a customer
// says they want, so a SKU's category and a lead's stated interest read as the same language.
...
export const PRODUCT_CATEGORY_LABEL = Object.fromEntries(PRODUCT_CATEGORY.map((p) => [p.v, p.l]));
