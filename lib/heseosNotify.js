...
// for the customer's first name) — approved in Meta Business Manager on the second attempt; the
// first wording ("Thank you for your interest in HESEOS smart home solutions") read as
// promotional to Meta's classifier and got flagged for Marketing, same thing that happened to
// 'newpartner_onboarding_welcome' (see notifyHeseosPartnerQrClaimed's comment). The `body` text
// below must match the approved template's wording exactly — Meta rejects the send outright if
// the text sent doesn't match what was approved, not just if the template doesn't exist.
export async function notifyHeseosMetaLeadCaptured(lead) {
  if (!lead?.phone) return { ok: false, error: 'Lead has no phone number' };
  const firstName = lead.name ? String(lead.name).trim().split(/\s+/)[0] : 'there';
  const body = [
    `Hi ${firstName}! 👋 We've received your enquiry with HESEOS.`,
    '',
