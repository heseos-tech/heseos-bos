...
export async function notifyHeseosLeadAdded(lead, addedByLabel) {
  ...
  return sendHeseosWhatsAppMessage(lead.phone, body);
}
...
export async function notifyHeseosPartnerQrClaimed(partner, employeeId) {
  ...
  if (partner?.phone) {
    ...
    partnerResult = await sendHeseosWhatsAppMessage(partner.phone, partnerBody);
  }
  ...
  if (employee?.phone) {
    ...
    employeeResult = await sendHeseosWhatsAppMessage(employee.phone, employeeBody);
  }
