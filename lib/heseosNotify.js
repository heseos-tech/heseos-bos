...
//
// Needs a WhatsApp template approved in Meta Business Manager first (Business Settings -> WhatsApp
// Manager -> Message Templates) named exactly 'meta_lead_welcome', category Utility, one
// {{1}} variable (the customer's first name), matching the `body` text below — Meta rejects the
// send until that template exists and is Approved, same as every other template used in this
// file (see e.g. notifyHeseosPartnerQrClaimed's comment on 'partner_onboarding_message' for how
// that approval process tends to go). Until then this fails quietly, like every export here.
export async function notifyHeseosMetaLeadCaptured(lead) {
  if (!lead?.phone) return { ok: false, error: 'Lead has no phone number' };
  const firstName = lead.name ? String(lead.name).trim().split(/\s+/)[0] : 'there';
  const body = [
    `Hi ${firstName}! 👋 Thank you for your interest in HESEOS smart home solutions.`,
    '',
    'One of our customer experience representatives will get in touch with you shortly.',
    '',
    '— Team HESEOS',
  ].join('\n');
  return sendHeseosTemplateMessage(
