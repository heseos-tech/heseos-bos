...
export function describeLeadOrigin(lead, { partners = [], employees = [], leads = [], links = [] } = {}) {
  const partnerName = (id) => {
    ...
    return p?.shopName || p?.businessName || 'one of our partners';
  };
...
// Same lookup precedence as describeLeadOrigin above, but phrased for the CUSTOMER themselves
// rather than for a partner/employee reading about someone else's lead — e.g. "Rohan referred
// you earlier" instead of "...referred them earlier". Used by lib/heseosReturningFlow.js's
// "welcome back" greeting (via lib/heseosLeadSync.js's heseosLeadOriginNote) when a customer
// who scans a QR code / taps a referral link (or just messages in) already has an active lead,
// so they're told plainly that they're already in the system and how that happened, instead of
// being walked through the full new-enquiry questionnaire again.
export function describeLeadOriginForCustomer(lead, { partners = [], employees = [], leads = [], links = [] } = {}) {
  const partnerName = (id) => {
    const p = partners.find((p) => p.id === id);
    return p?.shopName || p?.businessName || 'one of our partners';
  };
  ...
  if (lead.attributionKind === 'referral_customer') {
    const referrer = lead.referredByLeadId ? leads.find((l) => l.id === lead.referredByLeadId) : null;
    return referrer ? `${referrer.name} referred you earlier` : 'a friend referred you earlier';
  }
  if (lead.attributionKind === 'qr_location') {
    return `it was scanned via the '${linkLabel(lead.attributionLinkId)}' QR code earlier`;
  }
  if (lead.partnerId) {
    return `our partner ${partnerName(lead.partnerId)} added it earlier`;
  }
  if (lead.addedByEmployeeId) {
    return `our team member ${employeeName(lead.addedByEmployeeId)} added it earlier`;
  }
  if (lead.source === 'whatsapp_bot') {
    return 'you messaged us directly on WhatsApp earlier';
  }
  const label = LEAD_SOURCES[lead.source];
  return label ? `it came in via ${label}` : "it's already with us";
}
