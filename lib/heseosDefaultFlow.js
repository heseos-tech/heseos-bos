...
// of that change.
//
// ONLY reached via an actual QR-scan/referral-link attribution match (see triggers below) — a
// brand-new chat with no attribution and no existing lead gets lib/heseosOrganicFlow.js's
// lighter menu instead, which is now the fallback default. A phone number that already has an
// existing lead (from ANY source, attributed or not) always overrides straight to
// lib/heseosReturningFlow.js regardless of what's picked here — see the webhook's
// `existingLeadId` handling.
//
...
function buildHeseosDefaultFlow(tenantId) {
  ...
  const nodes = [
    { id: 'start', type: 'start', x: 60, y: 320, data: {} },
    {
      ...
      data: {
        ...
        // HESEOS brand poster — sent as its own image message 4s after this greeting, then the
        // very next node (n_ask_smart, below) follows immediately with no further delay. See
        // lib/botFlowEngine.js's play() for what data.imageAfter does; this is the ONLY place
        // (along with lib/heseosOrganicFlow.js's matching n_greet) that sets it — not a Flow
        // Builder field, just this flow's own seeded data.
        imageAfter: { path: '/whatsapp/heseos-welcome.jpg', delayMs: 4000 },
      },
    },
    {
    ...
    },
    {
      ...
      data: {
        text: "Wonderful! 🎉 Let's get started — what's your *full name*?", fieldKey: 'name',
        // Sends a chat whose QR/referral link already carries a known city (a qr_partner or
        // qr_location scan, or a referral_partner link — see lib/attribution.js's
        // attributionCityFor) to the city-CONFIRMATION menu below instead of the plain open
        // question — everyone else (organic chat, or a referral_customer link, which has no
        // business/location city to infer) just keeps following the normal 'default' edge to
        // n_ask_city, exactly as before this existed. See evaluateChatRedirect.
        altIf: 'hasAttributionCity', altHandle: 'alt_confirm_city',
      },
    },
    {
    ...
    {
      // Only ever reached when this chat's link already tells us a city — a partner's QR/
      // referral (their own profile city) or a location QR (the city captured on that placement
      // itself) — confirms it instead of asking the customer to re-type a city HESEOS can
      // already infer from how they got here. 'Yes' saves that city straight onto the lead (its
      // valueTemplate resolves {{attributionCity}} instead of using the option's own "Yes"
      // label — see the menu-answer handling in lib/botFlowEngine.js) and skips straight to
      // property type; 'No' falls through to the exact same open question everyone else gets
      // (n_ask_city_other, sharing n_ask_city's own wording).
      id: 'n_confirm_city', type: 'menu', x: 1180, y: 60,
      data: {
        text: 'Thanks, {{name}}! 😊 You want automation in *{{attributionCity}}*?',
        fieldKey: 'city',
        options: [
          { id: 'opt_city_yes', label: 'Yes', valueTemplate: '{{attributionCity}}' },
          { id: 'opt_city_no', label: 'No, different city' },
        ],
      },
    },
    {
      id: 'n_ask_city_other', type: 'question', x: 1180, y: 380,
      data: { text: 'No problem! Please tell me the *city* where you want automation.', fieldKey: 'city' },
    },
    {
  ...
  const edges = [
    ...
    { id: 'e_name_confirm_city', source: 'n_ask_name', sourceHandle: 'alt_confirm_city', target: 'n_confirm_city' },
    { id: 'e_city', source: 'n_ask_city', sourceHandle: 'default', target: 'n_ask_propertytype' },
    { id: 'e_confirm_city_yes', source: 'n_confirm_city', sourceHandle: 'opt_city_yes', target: 'n_ask_propertytype' },
    { id: 'e_confirm_city_no', source: 'n_confirm_city', sourceHandle: 'opt_city_no', target: 'n_ask_city_other' },
    { id: 'e_city_other', source: 'n_ask_city_other', sourceHandle: 'default', target: 'n_ask_propertytype' },
  ...
  return {
    ...
    enabled: true,
    triggers: { keywords: [], attribution: ['qr', 'referral'], isDefault: false },
    nodes,
    edges,
    createdAt: now,
    updatedAt: now,
...
// One-off migration, applied in place: n_greet's data.imageAfter (the HESEOS brand poster —
// see lib/botFlowEngine.js's play()) was added after this flow first shipped, so an already-
// seeded tenant's row wouldn't otherwise ever pick it up. Patches ONLY that one field onto that
// one node, leaving everything else — including any text/node/edge changes an admin has since
// made in Flow Builder — completely untouched. Safely a no-op once applied once, and a no-op
// (never throws) if n_greet was itself since deleted or renamed.
function withHeseosGreetImage(nodes) {
  const greet = (nodes || []).find((n) => n.id === 'n_greet');
  if (!greet || greet.data?.imageAfter) return null;
  return (nodes || []).map((n) => (n.id === 'n_greet'
    ? { ...n, data: { ...n.data, imageAfter: { path: '/whatsapp/heseos-welcome.jpg', delayMs: 4000 } } }
    : n));
}

export async function ensureHeseosDefaultFlow(tenant, existingFlows) {
  const existing = (existingFlows || []).find((f) => f.id === HESEOS_DEFAULT_FLOW_ID);
  if (existing) {
    const migratedNodes = withHeseosGreetImage(existing.nodes);
    if (!migratedNodes) return existingFlows;
    await dbPatch('bot_flows', HESEOS_DEFAULT_FLOW_ID, { nodes: migratedNodes });
    return existingFlows.map((f) => (f.id === HESEOS_DEFAULT_FLOW_ID ? { ...f, nodes: migratedNodes } : f));
  }
