  ...
  const nodes = [
    ...
    },
    {
      ...
      data: {
        ...
        // Sends a genuine QR-scan chat (see lib/attribution.js's partnerCityFor) to the
        // city-CONFIRMATION menu below instead of the plain open question — everyone else
        // (no partner city on file) just keeps following the normal 'default' edge to
        // n_ask_city, exactly as before this existed. See evaluateChatRedirect.
        altIf: 'hasPartnerCity', altHandle: 'alt_confirm_city',
      },
    },
    {
    ...
    {
      // Only ever reached for a genuine qr_partner scan with a city on the partner's own
      // profile — confirms it instead of asking the customer to re-type a city HESEOS can
      // already infer from WHICH partner's QR they just scanned. 'Yes' saves that city straight
      // onto the lead (its valueTemplate resolves {{partnerCity}} instead of using the option's
      // own "Yes" label — see the menu-answer handling in lib/botFlowEngine.js) and skips
      // straight to property type; 'No' falls through to the exact same open question everyone
      // else gets (n_ask_city_other, sharing n_ask_city's own wording).
      id: 'n_confirm_city', type: 'menu', x: 1180, y: 60,
      data: {
        text: 'Thanks, {{name}}! 😊 You want automation in *{{partnerCity}}*?',
        fieldKey: 'city',
        options: [
          { id: 'opt_city_yes', label: 'Yes', valueTemplate: '{{partnerCity}}' },
