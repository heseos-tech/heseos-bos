  ...
  const nodes = [
    { id: 'start', type: 'start', x: 60, y: 320, data: {} },
    {
      ...
      data: {
        ...
        // HESEOS brand poster — sent as its own image message right after this greeting, no
        // delay (see HESEOS_GREET_IMAGE above), then the very next node (n_ask_smart, below)
        // follows immediately too. See lib/botFlowEngine.js's play() for what data.imageAfter
        // does; this is the ONLY place (along with lib/heseosOrganicFlow.js's matching
        // n_greet) that sets it — not a Flow Builder field, just this flow's own seeded data.
...
// in place — necessary because this flow is seeded once into Postgres and never overwritten, so
// a code change to HESEOS_GREET_IMAGE alone wouldn't otherwise ever reach a tenant whose flow
// already exists (ours). Patches ONLY that one field onto that one node, leaving everything
// else — including any text/node/edge changes an admin has since made in Flow Builder —
// completely untouched. A no-op once it matches, and a no-op (never throws) if n_greet was
// itself since deleted or renamed.
