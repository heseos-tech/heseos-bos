...
//
// A code is embedded in the WhatsApp pre-filled message text as `(ref:<code>)`, exactly like
// the older app/wa/[ref] shop-QR mechanism, so app/api/bot/webhook/route.js's bridgeToHeseosLeads
// can pull it back out of the first inbound message and resolve it here.
...
// Whether a kind is a QR code (scanned) vs. a referral link (shared/clicked) — purely cosmetic,
// used to pick copy/labels in the UI ("Scans" vs "Clicks").
...
export async function getHeseosBotTenant() {
  ...
  return all.find((t) => t.linkToHeseosLeads === true) || null;
}
