...
//  - qr_location: label + pincode (both required — e.g. "Koramangala Billboard", "560034") so a
//    placement can be reported on by area, not just by its own name
//  - referral_customer: customerLeadId + customerName + customerPhone (the referring customer)
export async function createAttributionLink({ kind, label, pincode, partnerId, customerLeadId, customerName, customerPhone, createdBy, batchLabel }) {
