...
// By deliberate choice (not an oversight — see the git history if you're wondering), every QR
// code / referral link is a DIRECT https://wa.me/... deep link, never routed through our own
// domain first. That means a scan/click is never logged server-side (recordVisit, below, is
// only ever reached via the legacy app/go/[code] redirector, which nothing new points at
// anymore) — but the person tapping a code lands in WhatsApp instantly, with no intermediate
// hop at all. Lead attribution is unaffected either way: it comes from the "(ref:<code>)" tag
// in the message text itself, not from however the person got to WhatsApp.
//
// app/go/[code] is kept around (not deleted) purely so any already-printed sticker whose QR
// image happens to encode an old /go/<code> URL keeps working — it still logs a visit and
// redirects correctly. Nothing generates new links in that shape anymore.
