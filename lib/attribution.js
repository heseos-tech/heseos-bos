...
// The city on file for the partner whose QR was actually SCANNED (qr_partner only —
// referral_partner is a shared LINK, not a scan, so it's deliberately excluded here even though
// referrerNoteFor treats the two the same for wording purposes: a link click carries no
// "which sticker" signal the way a scan does). Used by the webhook's new-chat branch to
// pre-fill {{partnerCity}} for lib/heseosDefaultFlow.js's city-confirmation step, so a customer
// scanning a partner's sticker can just confirm the city instead of typing one HESEOS already
// knows from the partner's own profile. Returns '' when there's no partner, no city on file, or
// the link isn't actually a QR scan. Never throws.
export async function partnerCityFor(link) {
  if (!link || link.kind !== 'qr_partner' || !link.partnerId) return '';
  try {
...
export async function referrerNoteFor(link) {
  ...
  try {
    if (link.kind === 'qr_partner' || link.kind === 'referral_partner') {
      ...
      const who = partner?.businessName || partner?.name || link.label || 'a friend';
