...
import { findFirstLeadByPhone, describeLeadOrigin, describeLeadOriginForCustomer } from '@/lib/leadOrigin';
import { autoAssignByCity } from '@/lib/leadAssign';
import { notifyHeseosPartnerQrLeadCreated } from '@/lib/heseosNotify';
...
export async function finalizeHeseosLead(tenant, chat) {
  ...
  // Tells the partner in real time that their QR code just produced a lead — lead.partnerId is
  // only ever set by createHeseosLead's "genuinely first lead for this phone number" branch, so
  // this can never double-fire for the same customer's later messages, and the link.kind check
  // keeps it to an actual QR-code SCAN rather than a shared referral LINK (referral_partner —
  // see lib/heseosNotify.js's notifyHeseosPartnerQrLeadCreated for why that's deliberate). Never
  // allowed to affect lead creation itself, same "deliberately never throws" contract as every
  // other lib/heseosNotify.js export.
  if (lead.partnerId && link?.kind === 'qr_partner') {
    try {
      const partner = await dbGetById('partners', lead.partnerId);
      if (partner) await notifyHeseosPartnerQrLeadCreated(partner, lead);
    } catch (err) {
      console.error('notifyHeseosPartnerQrLeadCreated error:', err);
    }
  }
  ...
  // The customer dropped out (or went off-script, forcing a handoff) before ever reaching the
  // city question — chat.answers.city is unset, so the loop above left patch.city unset too.
  // Rather than hand a sales rep a lead with zero location on it, fall back to whatever city we
  // already inferred from the QR code / referral link they arrived through — chat.attributionCity,
  // set at chat-creation time (see app/api/bot/webhook/route.js + lib/attribution.js's
  // attributionCityFor: the partner's own profile city for a qr_partner/referral_partner scan,
  // or the location link's own city for qr_location). Blank for a referral_customer link or no
  // link at all, in which case this is a no-op and the lead stays unassigned-by-city same as
  // before — there's nothing to infer it from.
  if (!patch.city && chat.attributionCity) {
    patch.city = chat.attributionCity;
  }
...
// Customer-facing "why am I already in your system" sentence — powers lib/heseosReturningFlow.js's
// "welcome back" greeting via the {{leadOriginNote}} template var, right alongside
// {{leadSummary}} above. Tells the customer plainly HOW their existing lead got here (scanned a
// partner's QR code, came in via a referral link, a partner/team member punched it in by hand,
// etc — lib/leadOrigin.js's describeLeadOriginForCustomer) instead of silently re-running the
// full intake questionnaire as if this were their first time reaching out. Computed fresh each
// time, same reasoning as heseosLeadSummary above. Never throws: a lookup hiccup here must never
// break the greeting itself, it just falls back to no origin sentence.
export async function heseosLeadOriginNote(lead) {
  if (!lead) return '';
  try {
    const [partners, employees, links, leads] = await Promise.all([
      dbList('partners'), dbList('employees'), dbList('attribution_links'), dbList('leads'),
    ]);
    return describeLeadOriginForCustomer(lead, { partners, employees, links, leads });
  } catch (err) {
    console.error('heseosLeadOriginNote error:', err);
    return '';
  }
}
...
export async function scheduleHeseosDemo(chat) {
  ...
  try {
    ...
    const patch = {
      ...
      // Same reset as app/api/leads/[id]/route.js's 'scheduleDemo' PATCH branch — a brand-new
      // slot, booked or moved via WhatsApp this time, needs the same fresh eligibility for the
      // no-answer cadence and the demo reminders (see app/api/cron/lead-reminders/route.js).
      noAnswerAttempts: 0,
...
// Persists a customer's reply to lib/heseosRatingFlow.js — safe to call unconditionally from
// every handoff (same "no-op unless this chat's answers actually hold what I need" contract as
// scheduleHeseosDemo/createHeseosFollowUpLead above). Only ever has both `rating` (validated
// 1-5 by the flow's own QUESTION_VALIDATORS entry — see lib/botFlowEngine.js) and `ratingRole`
// set when the chat was deliberately routed into the rating flow by
// lib/heseosNotify.js's activateHeseosRatingFlow, right after sending the WhatsApp message that
// asks for it — see that function's own comment for why role/employee name travel as answers
// rather than needing their own chat columns.
export async function submitHeseosRating(chat) {
  const { rating, ratingRole, ratingEmployeeName, ratingComment } = chat?.answers || {};
  if (!rating || !ratingRole || !chat?.leadId) return false;
  try {
    const lead = await dbGetById('leads', chat.leadId);
    if (!lead) return false;
    const now = new Date().toISOString();
    const comment = ratingComment && String(ratingComment).trim().toLowerCase() !== 'skip' ? String(ratingComment).trim() : '';
    const roleLabel = ratingRole === 'salesEngineer' ? 'Sales Engineer' : 'Pre-Sales';
    const patch = ratingRole === 'salesEngineer'
      ? { salesEngineerRating: Number(rating), salesEngineerRatingNote: comment, salesEngineerRatingAt: now }
      : { preSalesRating: Number(rating), preSalesRatingNote: comment, preSalesRatingAt: now };
    patch.history = pushHistory(lead, {
      event: `${roleLabel}${ratingEmployeeName ? ` (${ratingEmployeeName})` : ''} rated ${rating}/5 by customer`,
...
// Persists a customer's reply to lib/heseosNoAnswerFlow.js — the WhatsApp side of the
// no-answer follow-up sequence (see that file's header comment for the full trigger chain:
// app/api/cron/lead-reminders/route.js notices a stale Call Not Picked lead, sends the nudge
// template, then lib/heseosNotify.js's activateHeseosNoAnswerFlow routes this chat here). Same
// "no-op unless this chat's answers actually hold what I need" contract as submitHeseosRating
// above. Guarded on the lead STILL being in 'call_not_picked' — if pre-sales already moved it
// on (reached the customer directly, scheduled a demo, etc.) by the time this late reply comes
// in, that human update wins and this is a safe no-op rather than clobbering newer state with a
// stale WhatsApp reply.
export async function resolveHeseosNoAnswerReply(chat) {
  const { noAnswerReply } = chat?.answers || {};
  if (!noAnswerReply || !chat?.leadId) return false;
  try {
    const lead = await dbGetById('leads', chat.leadId);
    if (!lead || lead.contactStage !== 'call_not_picked') return false;
    const now = new Date().toISOString();
    const patch = noAnswerReply === 'interested'
      ? {
          contactStage: 'follow_up',
          contactStageAt: now,
          followUpAt: now,
          contactNote: 'Replied to automated follow-up — still interested, please call back',
          noAnswerAttempts: 0,
          noAnswerLastAttemptAt: null,
        }
      : {
          ...
          contactStageAt: now,
          contactRejectReason: 'no_response',
          noAnswerAttempts: 0,
          noAnswerLastAttemptAt: null,
        };
    patch.history = pushHistory(lead, {
      event: noAnswerReply === 'interested' ? 'Replied to automated follow-up — still interested' : 'Replied to automated follow-up — not interested',
      by: 'Customer (WhatsApp)',
    });
    await dbPatch('leads', chat.leadId, patch);
    return true;
...
// Flags a lead the moment a customer asks to move an already-booked demo via WhatsApp
// (lib/heseosReturningFlow.js's "Yes, change it" branch — see that handoff node's
// flagRescheduleRequested: true, threaded through by lib/botFlowEngine.js's endHandoff exactly
// like a handoff's optional leadHistoryEvent). Purely a visibility flag for
// components/employee/PresalesPanel.jsx / SalesEngineerPanel.jsx to surface on the lead card —
// the history entry for "customer requested a reschedule" is already pushed separately via the
// existing leadHistoryEvent mechanism on that same handoff node, so this only ever sets the
// timestamp, nothing else. Cleared the moment the slot is actually moved (see
// app/api/leads/[id]/route.js's 'reschedule'/'scheduleDemo' branches and the reschedule-kind
// demoOutcome branch).
export async function flagHeseosRescheduleRequested(chat) {
  if (!chat?.leadId) return false;
  try {
    await dbPatch('leads', chat.leadId, { rescheduleRequestedAt: new Date().toISOString() });
    return true;
  } catch (err) {
...
// Same idea as LEAD_CONDITIONS/evaluateLeadRedirect just above, but for a 'question' node's
// own outgoing edge instead of a menu option's — and checked against the CHAT itself rather
// than a lead it's linked to, since this needs to run early in a fresh enquiry, well before any
// lead exists (finalizeHeseosLead only ever runs at handoff — see lib/botFlowEngine.js). Today's
// only user: lib/heseosDefaultFlow.js's n_ask_name node, which takes its altHandle straight to
// a city-CONFIRMATION menu (n_confirm_city) instead of the plain open city question whenever
// this chat's attributionCity was already resolved at chat-creation time (a qr_partner/
// qr_location scan or a referral_partner link with a known city — see lib/attribution.js's
// attributionCityFor) — everyone else falls straight through to the node's normal 'default'
// edge, unchanged.
const CHAT_CONDITIONS = {
  hasAttributionCity: (chat) => !!(chat && chat.attributionCity),
};

export async function evaluateChatRedirect(chat, node) {
  const alt = node?.data;
  if (!alt?.altHandle || !alt?.altIf) return null;
  const check = CHAT_CONDITIONS[alt.altIf];
  if (!check || !check(chat)) return null;
  return { handle: alt.altHandle };
}
...
// Called when a returning customer explicitly says they have a NEW requirement — see
// lib/heseosReturningFlow.js's "I have a new requirement" branch. Creates a genuinely SEPARATE
// lead row for the fresh enquiry rather than folding a text note onto whatever old lead this
// chat was linked to (that old lead might already be Converted or Rejected — done, one way or
// the other — or might still be an active pending enquiry; either way, "I have a new
// requirement" means a second, distinct ask from the same customer, which deserves its own
// pipeline entry a presales rep can work independently). Reuses createHeseosLead's own
// first-touch-attribution dedup (it does its own findFirstLeadByPhone check), so this still can
// never hand out fresh referral/QR credit just because the customer is describing something new
// — same "whoever brought them in first" rule as everywhere else. Inherits the OLD lead's city
// if it had one (a reasonable default — same customer, presumably same location) so it can be
// auto-assigned immediately, the same way Meta/Google/website leads already are at creation.
export async function createHeseosFollowUpLead(tenant, chat) {
  if (!chat?.answers?.newRequirement) return null;
  try {
    const priorLead = await fetchLinkedLead(chat);
    const lead = await createHeseosLead(tenant, { phone: chat.phone, name: chat.answers?.name || chat.name, link: null });
    const patch = {};
    let history = pushHistory(lead, {
      event: 'New requirement (via WhatsApp)',
      by: 'HESEOS Buddy (WhatsApp)',
      note: chat.answers.newRequirement,
    });
    if (priorLead?.city) {
      patch.city = priorLead.city;
      try {
        const { assignedTo, salesEngineerId } = await autoAssignByCity(priorLead.city);
        if (assignedTo) {
          patch.assignedTo = assignedTo;
          patch.salesEngineerId = salesEngineerId;
          history = pushHistory({ ...lead, history }, { event: 'Auto-assigned by city', by: 'system', note: `${priorLead.city} · pre-sales matched` });
        }
      } catch (err) {
        console.error('createHeseosFollowUpLead auto-assign error:', err);
      }
    }
