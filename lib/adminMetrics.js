...
import { displayStatus, isDeadLead, CONTACT_REJECT_REASONS, DEMO_REJECT_REASONS } from './leadStage';
...
// Averages one rating field (preSalesRating / salesEngineerRating) over an employee's
// assigned leads — only leads a customer actually replied 1-5 to count, so an employee with
// zero replies gets `avgRating: null` (never a misleading 0) rather than being folded into the
// average as if they'd been rated poorly. Shared by engineerStats and presalesStats below —
// see lib/heseosLeadSync.js's submitHeseosRating for where these fields get written, and
// components/admin/ui.jsx's RatingBadge for how this renders.
function ratingSummary(assignedLeads, field) {
  const ratings = assignedLeads.map((l) => Number(l[field])).filter((r) => Number.isFinite(r) && r > 0);
...
export function engineerStats(engineer, leads) {
  ...
  return { assigned: assigned.length, demosDone, quotationsSent, conversions, conversionRate: rate, demosThisWeek, ...ratingSummary(assigned, 'salesEngineerRating') };
}
...
export function presalesStats(emp, leads) {
  ...
  return { assigned: assigned.length, callsMade, demosScheduled, demosCompleted, conversions: converted, conversionRate: rate, ...ratingSummary(assigned, 'preSalesRating') };
}
...
// demo hasn't yet been marked won/lost. Every dead lead (isDeadLead — a pre-demo "Not
// Interested" just as much as a post-demo rejection) shows the same "Lost" label here,
// regardless of which specific reason displayStatus() would otherwise show — checking for the
// literal label 'Rejected' missed the pre-demo case entirely, since displayStatus() returns
// the contactStage's own label ("Not Interested") for that one rather than ever falling
// through to the canonical stage name.
export function adminStatus(lead) {
  ...
  return isDeadLead(lead) ? { ...st, label: 'Lost' } : st;
}
...
// ── Funnel drop-off by reason (Reports) ────────────────────────────────────
// Two separate breakdowns, not one combined list — contactRejectReason (pre-sales, set only
// when contactStage is marked Not Interested) and demoRejectReason (post-demo, set on either
// dead demoOutcome) are different taxonomies for different funnel stages (see
// lib/leadStage.js's CONTACT_REJECT_REASONS/DEMO_REJECT_REASONS), so mixing their counts into
// one chart would compare apples to oranges. Rows are always returned in the reason list's own
// fixed declared order (never re-sorted by count) so a reason's bar color/position never shifts
// just because a date-range filter changed which reason happens to be most common right now —
// same fixed-order-by-identity rule as sourceBreakdown/revenueBySource above, just without
// needing per-row color assignment since components/admin/ui.jsx's <Funnel> already colors by
// row position, which this keeps stable.
function reasonRows(leads, field, reasonList) {
  const counts = {};
  for (const l of leads) {
    if (l[field]) counts[l[field]] = (counts[l[field]] || 0) + 1;
  }
  const total = Object.values(counts).reduce((a, b) => a + b, 0);
  return reasonList.map((r) => ({
    key: r.key,
    label: r.label,
    count: counts[r.key] || 0,
    pct: total ? Math.round(((counts[r.key] || 0) / total) * 100) : 0,
  }));
}

export function dropOffByReason(leads) {
  return {
    preSales: reasonRows(leads, 'contactRejectReason', CONTACT_REJECT_REASONS),
    postDemo: reasonRows(leads, 'demoRejectReason', DEMO_REJECT_REASONS),
  };
}
