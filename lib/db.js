...
const ALLOWED = new Set([
  ...
  'bot_flows',       // Heseos Bot platform — one row per tenant's self-built visual flow (Flow Builder)
  'rate_limits',     // generic per-key hit counter (see lib/rateLimit.js) — e.g. per-IP signup throttling
  'attribution_links',  // QR codes + referral links (partner QR, location QR, partner referral, customer referral)
  'attribution_visits', // one row per scan/click of an attribution_links code — raw funnel event log
  'products',        // catalogue: SKUs, pricing, photos — feeds the quotation builder and the Partner/Team catalogue view
  'tasks',           // assignable follow-up tasks/reminders for the team (Admin -> Tasks, and each employee's My Tasks)
  'payouts',         // partner payout/settlement ledger (Admin -> Payouts) — tracking only, never an actual money-transfer action
]);
function tbl(t) {
  ...
  if (!_schemaReady) {
    _schemaReady = (async () => {
      await Promise.all([
        ...
        sql`CREATE TABLE IF NOT EXISTS attribution_links (
              id TEXT PRIMARY KEY,
              created_at TIMESTAMPTZ DEFAULT now(),
              data JSONB NOT NULL
            )`,
        sql`CREATE TABLE IF NOT EXISTS attribution_visits (
              id TEXT PRIMARY KEY,
              created_at TIMESTAMPTZ DEFAULT now(),
              data JSONB NOT NULL
            )`,
      ...
      await Promise.all([
        ...
        sql`CREATE INDEX IF NOT EXISTS idx_bottenant_approval ON bot_tenants ((data->>'approvalStatus'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_bottenant_wapid   ON bot_tenants ((data->>'waPhoneNumberId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_bottenant_waverify ON bot_tenants ((data->>'waVerifyToken'))`,
        ...
        sql`CREATE INDEX IF NOT EXISTS idx_botflow_tenant    ON bot_flows ((data->>'tenantId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_leads_attrlink    ON leads ((data->>'attributionLinkId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_attrlink_kind     ON attribution_links ((data->>'kind'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_attrlink_partner  ON attribution_links ((data->>'partnerId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_attrvisit_link    ON attribution_visits ((data->>'linkId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_products_active   ON products ((data->>'active'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_products_category ON products ((data->>'category'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_tasks_assignee     ON tasks ((data->>'assigneeId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_tasks_status       ON tasks ((data->>'status'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_tasks_lead         ON tasks ((data->>'leadId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_payouts_partner    ON payouts ((data->>'partnerId'))`,
        sql`CREATE INDEX IF NOT EXISTS idx_payouts_status     ON payouts ((data->>'status'))`,
      ]);
...
const FILES = {
  ...
  bot_flows: path.join(process.cwd(), 'data', 'bot_flows.json'),
  rate_limits: path.join(process.cwd(), 'data', 'rate_limits.json'),
  attribution_links: path.join(process.cwd(), 'data', 'attribution_links.json'),
  attribution_visits: path.join(process.cwd(), 'data', 'attribution_visits.json'),
  products: path.join(process.cwd(), 'data', 'products.json'),
  tasks: path.join(process.cwd(), 'data', 'tasks.json'),
  payouts: path.join(process.cwd(), 'data', 'payouts.json'),
};
function readFile(t) {
