...
// ── Short-lived in-memory account cache ──────────────────────────────────────
// Vercel reuses ("warms") a serverless function instance across a burst of requests for a few
// minutes at a time — but every getEmployee()/getPartner()/getBotTenant() call still paid a
// fresh Neon round trip on every single request, even on a warm instance, because the cache()
// wrapper below only dedupes calls WITHIN one request (e.g. the (app) layout + the page), never
// across separate navigations/taps. This adds a short TTL cache on top of that: the same
// account, reused for a few seconds, so most taps in a session cost zero DB latency at all —
// only the first request to a freshly-warmed instance actually hits the database.
// Invalidated eagerly (see invalidateAccountCache) by every route that writes to that account,
// so an admin deactivating a partner/employee — or a partner editing their own profile — takes
// effect on their very next request rather than waiting out the TTL.
const ACCOUNT_TTL_MS = 20_000;
const accountCache = new Map(); // `${table}:${id}` -> { acct, expiresAt }

async function getCachedAccount(table, id) {
  const key = `${table}:${id}`;
  const hit = accountCache.get(key);
  if (hit && hit.expiresAt > Date.now()) return hit.acct;
  const acct = await dbGetById(table, id);
  accountCache.set(key, { acct, expiresAt: Date.now() + ACCOUNT_TTL_MS });
  return acct;
}

// Call after any write to a partner/employee/bot_tenant row so the next read reflects it
// immediately instead of serving a stale cached copy for up to ACCOUNT_TTL_MS.
export function invalidateAccountCache(table, id) {
  accountCache.delete(`${table}:${id}`);
}

// Wrapped in React's cache() so multiple calls within the same request (the (app) layout's
// auth guard, then the page itself) share one actual lookup instead of two — this was
// previously the single biggest cause of "every tap feels slow" in the Team/Partner apps, since
// every navigation re-ran this from scratch in both the shared layout and the page. The TTL
// cache above (getCachedAccount) then carries that same saving ACROSS separate navigations too.
export const getEmployee = cache(async function getEmployee() {
  ...
  if (!id) return null;
  const acct = await getCachedAccount('employees', id);
  return acct && acct.active !== false ? acct : null;
});

// See getEmployee() above for why this is wrapped in cache() + the TTL cache.
export const getPartner = cache(async function getPartner() {
  const token = (await cookies()).get(PARTNER_COOKIE)?.value;
  const id = decodeSession('partner', token);
  if (!id) return null;
  const acct = await getCachedAccount('partners', id);
  return acct && acct.active !== false ? acct : null;
});
...
export const getBotTenant = cache(async function getBotTenant() {
  const token = (await cookies()).get(BOT_COOKIE)?.value;
  const id = decodeSession('bot_tenant', token);
  if (!id) return null;
  const acct = await getCachedAccount('bot_tenants', id);
