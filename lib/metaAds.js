...
// Shared by two callers: the manual "Sync Leads Now" button in Settings, and the scheduled
// safety-net sync at app/api/cron/sync-meta-leads (an external scheduler hits that route on a
// timer, since Vercel's Hobby plan only allows once-daily cron). Safe to call any time — every
// lead is looked up by its deterministic id before inserting, so nothing is ever duplicated.
