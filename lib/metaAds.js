...
// on the Page was captured and the only "config" was env vars + Meta's own dashboard. The
// app-level webhook registration (appId/appSecret/verifyToken) is self-service the same way —
// see registerAppWebhook and app/api/admin/meta/route.js's save_app_credentials action — with
// the old META_APP_ID/META_APP_SECRET/META_LEAD_VERIFY_TOKEN/PUBLIC_BASE_URL env vars kept only
// as fallbacks for a deployment that already set them before this existed.
//
...
// has to type one in (or hit Meta’s 64-char cap by hand). Prefers an already-set
// META_LEAD_VERIFY_TOKEN env var once, so a deployment that configured one before this existed
// keeps using the exact same value Meta already has on file — falls back to a fresh random one
// otherwise. Once stored on the settings row, that's the only value ever used again.
async function ensureVerifyToken(settings) {
  if (settings?.verifyToken) return settings.verifyToken;
  const envToken = process.env.META_LEAD_VERIFY_TOKEN;
  const token = envToken && envToken.length <= 64 ? envToken : crypto.randomBytes(24).toString('hex');
...
// from Admin → Settings: appId/appSecret come from what the admin saved there (see
// save_app_credentials in app/api/admin/meta/route.js — falling back to the old
// META_APP_ID/META_APP_SECRET env vars for a deployment that set those before this existed),
// the verify token is generated and stored automatically (see ensureVerifyToken above), and the
// callback URL is built from `requestHost` — the Host header off the admin's own request to
// register this, since that's the one domain we know for certain reaches this deployment (a
// PUBLIC_BASE_URL env var, if still set, wins over that for an explicit override).
export async function registerAppWebhook(requestHost) {
  ...
  const baseUrl = process.env.PUBLIC_BASE_URL || (requestHost ? `https://${requestHost}` : null);
