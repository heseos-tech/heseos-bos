...
// {{leadSummary}} (Heseos's own tenant only, blank for everyone else) and {{partnerCity}} (a
// qr_partner scan's own city on file — see lib/attribution.js's partnerCityFor — blank for every
// other chat).
function templateVars(chat) {
  return {
    referrerNote: chat.referrerNote || '',
    leadSummary: chat.leadSummary || '',
    partnerCity: chat.partnerCity || '',
    ...(chat.answers || {}),
  };
}
...
export async function runFlowTurn(tenant, flow, chat, inboundText) {
  ...
  if (waiting.type === 'menu') {
    ...
    // '{{partnerCity}}'), resolved through the exact same template vars as any outbound message
