...
import { syncLeadField, finalizeHeseosLead, scheduleHeseosDemo, evaluateLeadRedirect, evaluateChatRedirect, appendHeseosLeadHistory, createHeseosFollowUpLead } from '@/lib/heseosLeadSync';
...
async function endHandoff(tenant, chat, closingText, historyEvent) {
  ...
  // demoDate/demoTime/demoAddress/newRequirement only ever mean anything for the ONE handoff
  // that just consumed them (above) — strip them from what gets persisted so a later, unrelated
  // handoff on this same chat (e.g. "check my enquiry status" reached after an earlier demo-
  // scheduling or new-requirement conversation) can never accidentally re-trigger scheduling or
  // follow-up-lead creation off stale leftover answers.
  const { demoDate, demoTime, demoAddress, newRequirement, ...restAnswers } = chat.answers || {};
